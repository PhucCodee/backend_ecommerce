...
{
    public class ProductQueryService(ApplicationDbContext context, IMapper mapper) : IProductQueryService
    {
        ...
        {
            var product = await GetProductsForDetail()
            ...
            List<int>? categoryIdSet)
        {
            if (p.MinPrice.HasValue)
                query = query.Where(x => x.ProductSkus.Any(s => s.IsDefault && s.Price >= p.MinPrice.Value));
            ...
            if (categoryIdSet is { Count: > 0 })
                query = query.Where(x => x.ProductCategories.Any(pc => categoryIdSet.Contains(pc.CategoryId)));
            ...
            if (!string.IsNullOrWhiteSpace(p.Status) && Enum.TryParse<ProductStatus>(p.Status, true, out var status))
            ...
            if (p.InStock.HasValue && p.InStock.Value)
                query = query.Where(x => x.ProductSkus
                    .Any(s => s.IsDefault && s.Inventory != null && s.Inventory.QuantityAvailable > 0));
        ...
        private static IQueryable<Domain.Entities.Product> ApplySorting(
            IQueryable<Domain.Entities.Product> query, string? sortBy, bool desc)
        {
